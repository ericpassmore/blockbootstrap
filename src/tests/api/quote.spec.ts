...
import { GET } from '../../routes/api/marketdata/quote/[ticker]/+server';

describe('GET /api/marketdata/quote/[ticker]', () => {
	it('returns 3 or more data rows and metadata.ticker is MSFT for valid request', async () => {
		const url = new URL(
			'http://localhost:5173/api/marketdata/quote/msft?from=2025-07-20&to=2025-08-17'
		);
